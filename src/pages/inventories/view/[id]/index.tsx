...
import { deleteFinanceById } from 'apiSdk/finances';
...
function InventoryViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<InventoryInterface>(
    ...
    () =>
      getInventoryById(id, {
        relations: ['organization', 'finance', 'order'],
      }),
  );

  const financeHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('finance', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/finances/create?inventory_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.finance?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/finances/view/${record.id}`)}
                            key={record.id}
                          >
              ...
              {hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/orders/create?inventory_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
